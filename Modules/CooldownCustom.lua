...
local function SuppressDebuffBorder(f)
    ...
    for i = 1, #borders do
        ...
        if border then 
            ...
            hooksecurefunc(border, "Show", SafeHide)
        end
    end

    if f.DebuffBorder and f.DebuffBorder.UpdateFromAuraData then
        hooksecurefunc(f.DebuffBorder, "UpdateFromAuraData", SafeHide)
    end
    ...
    for i = 1, select("#", f:GetRegions()) do
        ...
        if region and region.IsObjectType and region:IsObjectType("Texture") then
            if SafeEquals(region:GetAtlas(), "UI-HUD-CoolDownManager-IconOverlay") or SafeEquals(region:GetTexture(), 6707800) then
                region:SetAlpha(0)
                region:Hide()
                hooksecurefunc(region, "Show", SafeHide)
            end
        end
    end

    if f.PandemicIcon then 
        f.PandemicIcon:SetAlpha(0); f.PandemicIcon:Hide() 
        hooksecurefunc(f.PandemicIcon, "Show", SafeHide)
    end
    if type(f.ShowPandemicStateFrame) == "function" then
        hooksecurefunc(f, "ShowPandemicStateFrame", function(self) if self.PandemicIcon then self.PandemicIcon:Hide(); self.PandemicIcon:SetAlpha(0) end end)
    end
    if f.CooldownFlash then
        f.CooldownFlash:SetAlpha(0); f.CooldownFlash:Hide()
        hooksecurefunc(f.CooldownFlash, "Show", SafeHide)
        if f.CooldownFlash.FlashAnim and f.CooldownFlash.FlashAnim.Play then
    ...
    if f.SpellActivationAlert then 
        ...
        hooksecurefunc(f.SpellActivationAlert, "Show", SafeHide)
    end
...
local function DoLayoutBuffs(viewerName, key, isVertical)
    ...
    if isVertical then
        for i = 1, count do
            ...
            if growth == "UP" then
                f:SetPoint("CENTER", container, "CENTER", 0, yOffset)
            else
                f:SetPoint("CENTER", container, "CENTER", 0, -yOffset)
            end
        ...
        for i = 1, count do
            ...
            local xOffset = -(totalW / 2) + (w / 2) + (i - 1) * (w + gap)
            
            if growth == "LEFT" then
                f:SetPoint("CENTER", container, "CENTER", -xOffset, 0)
    ...
    local function LayoutViewer(viewer, cfg, cat)
        ...
        if attachToPlayer and _G.ElvUF_Player then
            ...
            for i = 1, count do
                ...
                f:SetSize(w, h)
                if i == 1 then
                    f:SetPoint("RIGHT", viewer, "RIGHT", 0, 0)
                else
                    local prev = cachedFrames[i-1]
                    f:SetPoint("RIGHT", prev, "LEFT", -gap, 0)
                end
            ...
            for i = 1, count do
                ...
                if growth == "LEFT" then
                    f:SetPoint("CENTER", viewer, "CENTER", -xOffset, 0)
                elseif growth == "RIGHT" then
                    f:SetPoint("CENTER", viewer, "CENTER", xOffset, 0)
                else
                    f:SetPoint("CENTER", viewer, "CENTER", xOffset, 0)
        ...
        if count > 0 then
            ...
            if cfgE.enableCustomLayout then
                ...
                for i = 1, r1c do
                    local f = cachedR1[i]
                    f:ClearAllPoints()
                    local xOffset = -(totalW1 / 2) + (w1 / 2) + (i - 1) * (w1 + gap)
                    f:SetPoint("CENTER", eViewer, "CENTER", xOffset, 0)
                    f:SetSize(w1, h1)
                ...
                for i = 1, r2c do
                    local f = cachedR2[i]
                    f:ClearAllPoints()
                    local xOffset = -(totalW2 / 2) + (w2 / 2) + (i - 1) * (w2 + gap2)
                    f:SetPoint("CENTER", _G.WishFlex_CooldownRow2_Anchor, "CENTER", xOffset, 0)
                    f:SetSize(w2, h2)
        ...
        if mixin then
            if mixin.OnCooldownIDSet then
                hooksecurefunc(mixin, "OnCooldownIDSet", function(frame) 
                    mod:ImmediateStyleFrame(frame, cat)
                    EventTrigger()
                end)
            end
            if mixin.OnActiveStateChanged then
                hooksecurefunc(mixin, "OnActiveStateChanged", function(frame) 
                    mod:ImmediateStyleFrame(frame, cat)
                    EventTrigger()
                end)
            end
    ...
    for vName, cat in pairs(viewers) do
        ...
        if v then
            if v.OnAcquireItemFrame then
                hooksecurefunc(v, "OnAcquireItemFrame", function(_, frame)
                    mod:ImmediateStyleFrame(frame, cat)
                    EventTrigger()
                end)
            end
            ...
            if v.itemFramePool then 
                if type(v.itemFramePool.Acquire) == "function" then hooksecurefunc(v.itemFramePool, "Acquire", EventTrigger) end
                if type(v.itemFramePool.Release) == "function" then hooksecurefunc(v.itemFramePool, "Release", EventTrigger) end
                if type(v.itemFramePool.ReleaseAll) == "function" then hooksecurefunc(v.itemFramePool, "ReleaseAll", EventTrigger) end
            end
        end
    end
